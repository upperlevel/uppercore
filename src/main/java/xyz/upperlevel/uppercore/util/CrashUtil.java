...
public final class CrashUtil {
	...
	public static FileWriter createCrashFile(Plugin plugin) {
		...
		try {
			...
			plugin.getLogger().severe("CANNOT CREATE LOG FILE, CHECK PERMISSION IN '" + plugin.getDataFolder()
					+ File.separator + CRASH_DIR + "'");
			return null;
		}
		try {
			return new FileWriter(file, false);
		} catch (IOException e) {
			plugin.getLogger().severe("CANNOT OPEN LOG FILE, CHECK PERMISSION IN '" + plugin.getDataFolder()
					+ File.separator + CRASH_DIR + "'");
			return null;
		}
	}
